...
4. Determine which provider IDs the user requested: `anthropic-api`,
   ...
   `gemini-api`, `github-copilot`, `chutes`, and `orcarouter` are also selectable, but they ship no
...
4. If the provider is registered but the model is not checked in, run
   ...
   OpenAI-compatible entry whose catalog price is zero plus its
   `orcarouter/free` alias; it still requires an OrcaRouter API key for
   inference and never turns the provider on implicitly.
