...
test("provider writes republish all installed targets and roll selection back on apply failure", async () => {
  ...
  assert.match(source, /runJson\(\["credential", id\], \{ stdin: credential/);
  ...
  assert.ok(removal.indexOf("updateProviderSelection(id, false)") < removal.indexOf('["credential", id, "--remove"]'));
