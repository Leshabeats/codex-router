...
export function registerIpcHandlers({ ipcMain, BrowserWindow, shell, fetchImpl = globalThis.fetch, senderGuard = () => true } = {}) {
  ...
  handleAction("saveProviderCredential", async ({ providerId, credential } = {}) => {
    ...
    await runJson(["credential", id], { stdin: credential, timeoutMs: 20_000 });
    await updateProviderSelection(id, true);
    return runJson(["providers"]);
  });
  handleAction("removeProviderCredential", async ({ providerId } = {}) => {
    const { id } = await validateProvider(providerId, "credential");
    // Remove the provider from every installed client before deleting its only
    // authentication path. If credential deletion then fails, the safely
    // disabled selection is deliberately left in place for an operator retry.
    await updateProviderSelection(id, false);
    return runJson(["credential", id, "--remove"], { timeoutMs: 20_000 });
  });
